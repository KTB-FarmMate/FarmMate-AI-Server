...
def add_exception_handlers(app: FastAPI):
    ...
    async def validation_exception_handler(request: Request, exc: ValueError):
        return JSONResponse(
            ...
            ).model_dump(exclude_none=True)  # 자동 필드 제거
        )
    ...
    async def http_exception_handler(request: Request, exc: HTTPException):
        # exc.detail이 dict인지 확인
        if isinstance(exc.detail, dict):
            error_code = exc.detail.get("errorClassName", get_error_code(exc.status_code))
            error_message = exc.detail.get("errorMessage", exc.detail.get("detail", ""))
        else:
        ...
        return JSONResponse(
            ...
            content=ApiResponse(
                message=get_status_message(exc.status_code),
                error=ErrorDetail(
                    code=error_code,
